...
int main(void)
{
  ...
  // HAL_GetTick() 기반 타이밍 변수
  uint32_t last_imu_time = 0;      // 마지막 IMU 읽기 시간
  uint32_t motor_start_time = 0;   // 모터 동작 시작 시간
  uint8_t motor_state = 0;         // 0: 전진, 1: 후진

  // 타이밍 상수
  const uint32_t IMU_INTERVAL = 10;     // 0.01초 = 10ms
  const uint32_t MOTOR_DURATION = 7000; // 7초 = 7000ms

  motor_start_time = HAL_GetTick();  // 시작 시간 기록
...
//     );
//
//     // 예: 서버 IP = 192.168.178.61
//     ESP_HTTP_Post_JSON("192.168.178.61", "/api/send/value", json_body);
//
//
//     // 2️⃣ GET 요청 및 값 파싱
...
//  ESP_HTTP_Post("192.168.178.61", "/data", "{\"id\":1,\"value\":42}");
  ...
  while (1)
  {
         ...
         // 2. IMU 데이터 읽기 및 전송 (0.01초마다)
         if (current_time - last_imu_time >= IMU_INTERVAL) {
             last_imu_time = current_time;

             // MPU6050 원본 int16_t 값 읽기
             MPU6050_Read_Accel(&Ax, &Ay, &Az);

             // 원본 값을 그대로 UART로 전송 (int16_t: -32768 ~ +32767)
